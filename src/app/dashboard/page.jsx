...
const MyBookingsPage = async () => {
    ...
    // console.log(session, "I am doing session");
    const user = session?.user;
    const res = await fetch (`http://localhost:5000/booking/${user?.id}`);
    const bookings = await res.json();
    // console.log(bookings ,"I am booking session");
            ...
            <div className="space-y-5">
                {
                    ...
                    <div key={booking._id} className="flex justify-between min-w-4xl items-center border-2 m-10 p-5 shadow-2xl rounded-2xl">
                        <div  className="flex gap-5">
                            <Image src={booking.image} alt={"booking.destinationName"} width={200} height={200}/> 
                            <div>
